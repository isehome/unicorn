-- Fix check_milestone_completion function to use delivered_confirmed instead of onsite_confirmed
-- The column was renamed in a previous migration
...
BEGIN
  CASE p_milestone_type
    ...
    -- Prewire Prep: All prewire items ordered AND delivered
    WHEN 'prewire_prep' THEN
      WITH prewire_stats AS (
        SELECT
          ...
          COUNT(*) FILTER (WHERE gp.required_for_prewire = true AND pe.delivered_confirmed = true) as delivered
        FROM project_equipment pe
        JOIN global_parts gp ON pe.global_part_id = gp.id
        WHERE pe.project_id = p_project_id
      )
      SELECT
        CASE WHEN total > 0 AND ordered = total AND delivered = total THEN true ELSE false END,
        CASE
          WHEN total = 0 THEN 0
          WHEN delivered = total THEN 100
        ...
        jsonb_build_object(
          ...
          'all_delivered', delivered = total AND total > 0
        )
      INTO is_complete, completion_percent, details
      FROM prewire_stats;
    ...
    -- Trim Prep: All non-prewire items ordered AND delivered
    WHEN 'trim_prep' THEN
      WITH trim_stats AS (
        SELECT
          ...
          COUNT(*) FILTER (WHERE (gp.required_for_prewire != true OR gp.required_for_prewire IS NULL) AND pe.delivered_confirmed = true) as delivered
        FROM project_equipment pe
        JOIN global_parts gp ON pe.global_part_id = gp.id
        WHERE pe.project_id = p_project_id
      )
...
DO $$
BEGIN
  RAISE NOTICE '✅ check_milestone_completion function updated';
  RAISE NOTICE 'Changed onsite_confirmed -> delivered_confirmed';
END $$;
