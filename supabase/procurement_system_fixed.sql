-- ============================================================
-- PROCUREMENT & SUPPLIER MANAGEMENT SYSTEM (FIXED)
-- Comprehensive supplier, purchase order, and tracking system
-- Version: 1.1 - Removed profile dependencies
-- ============================================================
...
create table if not exists public.suppliers (
  ...
  created_by text, -- Changed from uuid to text to avoid profile dependency
...
create table if not exists public.purchase_orders (
  ...
  submitted_by text, -- Changed from uuid to text
...
create table if not exists public.purchase_order_items (
  ...
  expected_delivery_date date,
  actual_delivery_date date,
  received_by text, -- Changed from uuid to text
...
create table if not exists public.shipment_tracking (
  ...
  notes text,
  added_by text, -- Changed from uuid to text
  created_at timestamptz default now(),
  updated_at timestamptz default now(),
