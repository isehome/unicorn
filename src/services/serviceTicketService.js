...
export const customerLookupService = {
  ...
  /**
   * Create a new contact (customer) from service ticket form
   * Checks for duplicates by phone or email before creating
   * @param {Object} contactData - Customer details
   * @returns {Object} The newly created contact with id, or existing contact if duplicate found
   */
  async createContact(contactData) {
    if (!supabase) throw new Error('Supabase not configured');

    const fullName = contactData.full_name?.trim() || contactData.customer_name?.trim();
    if (!fullName) {
      throw new Error('Customer name is required');
    }

    const email = contactData.email?.trim() || contactData.customer_email?.trim() || null;
    const phone = contactData.phone?.trim() || contactData.customer_phone?.trim() || null;

    try {
      // Check for existing contact by phone or email to prevent duplicates
      if (phone || email) {
        const conditions = [];
        ...
        if (existing) {
          console.log('[CustomerLookupService] Found existing contact:', existing.id);
          // Merge any new info into existing contact
          const updates = {};
          if (!existing.phone && phone) updates.phone = phone;
          if (!existing.email && email) updates.email = email;
          const address = contactData.address?.trim() || contactData.customer_address?.trim();
          ...
          if (Object.keys(updates).length > 0) {
            ...
            if (!updateError && updated) {
              console.log('[CustomerLookupService] Updated existing contact with new info');
              return updated;
            }
          }
          return existing;
      ...
      const firstName = contactData.first_name || nameParts[0] || '';
      const lastName = contactData.last_name || nameParts.slice(1).join(' ') || '';
      ...
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          email: email,
          phone: phone,
          address: contactData.address?.trim() || contactData.customer_address?.trim() || null,
      ...
      if (error) {
        console.error('[CustomerLookupService] Failed to create contact:', error);
