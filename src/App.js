...
export default App;t.text}`}>
        ...
        <div className="p-4 space-y-4">
          <input
            type="text"
            value={wireDropFormData.name}
            onChange={(e) => setWireDropFormData({...wireDropFormData, name: e.target.value})}
          ...
          <input
            type="text"
            value={wireDropFormData.uid}
            onChange={(e) => setWireDropFormData({...wireDropFormData, uid: e.target.value})}
            className={`w-full px-3 py-3 rounded-lg ${t.surface} ${t.text} border ${t.border}`}
            placeholder="UID *"
          />
          ...
          <input
            type="text"
            value={wireDropFormData.location}
            onChange={(e) => setWireDropFormData({...wireDropFormData, location: e.target.value})}
            className={`w-full px-3 py-3 rounded-lg ${t.surface} ${t.text} border ${t.border}`}
            placeholder="Location *"
          />
          
          <select
            value={wireDropFormData.type}
            onChange={(e) => setWireDropFormData({...wireDropFormData, type: e.target.value})}
            className={`w-full px-3 py-3 rounded-lg ${t.surface} ${t.text} border ${t.border}`}
          >
  ...
  const ProjectForm = () => {
    const handleSave = () => {
      if (!projectFormData.name || !projectFormData.wiringDiagramUrl || !projectFormData.portalProposalUrl) {
      ...
      if (selectedProject) {
        // Edit existing
        setProjects(prev => prev.map(p => p.id === selectedProject.id ? {...projectFormData, id: selectedProject.id} : p));
        alert('Project updated!');
      } else {
        // Add new
        const newProject = { ...projectFormData, id: Date.now().toString() };
      ...
      <div className={`min-h-screen ${t.bg}`}>
        ...
        <div className="p-4 space-y-4">
          ...
          <div className={`p-4 rounded-xl ${t.surface} border ${t.border}`}>
            ...
            <input
              type="url"
              value={projectFormData.wiringDiagramUrl}
              onChange={(e) => setProjectFormData({...projectFormData, wiringDiagramUrl: e.target.value})}
            ...
            <input
              type="url"
              value={projectFormData.portalProposalUrl}
              onChange={(e) => setProjectFormData({...projectFormData, portalProposalUrl: e.target.value})}
          ...
          <div className={`p-4 rounded-xl ${t.surface} border ${t.border}`}>
            ...
            <input
              type="url"
              value={projectFormData.oneDrivePhotos}
              onChange={(e) => setProjectFormData({...projectFormData, oneDrivePhotos: e.target.value})}
              className={`w-full px-3 py-3 rounded-lg ${t.surfaceHover} ${t.text} border ${t.border} mb-3`}
              placeholder="OneDrive Photos Folder URL"
            />
            <input
              type="url"
              value={projectFormData.oneDriveFiles}
              onChange={(e) => setProjectFormData({...projectFormData, oneDriveFiles: e.target.value})}
              className={`w-full px-3 py-3 rounded-lg ${t.surfaceHover} ${t.text} border ${t.border} mb-3`}
              placeholder="OneDrive Files Folder URL"
            />
            <input
              type="url"
              value={projectFormData.oneDriveProcurement}
              onChange={(e) => setProjectFormData({...projectFormData, oneDriveProcurement: e.target.value})}
              className={`w-full px-3 py-3 rounded-lg ${t.surfaceHover} ${t.text} border ${t.border}`}
              placeholder="OneDrive Procurement Folder URL"
            />
          </div>
  ...
  // Issue Detail View - Moved state outside
  ...
  const IssueDetailView = () => {
    if (!selectedIssue || !editedIssue) return null;
  ...
  // PM Project Detail State - moved outside
  const [editableProject, setEditableProject] = useState(null);

  useEffect(() => {
    if (selectedProject && currentView === 'pmProjectDetail') {
  ...
  const PMProjectDetail = () => {
    if (!selectedProject || !editableProject) return null;
