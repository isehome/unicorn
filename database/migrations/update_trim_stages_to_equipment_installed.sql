-- =====================================================
-- UPDATE TRIM STAGES TO USE EQUIPMENT INSTALLED STATUS
-- =====================================================
-- Changes the trim_stages_percentage calculation from:
--   "Wire drops with completed trim_out stages"
-- To:
--   "Equipment installed (via wire drop trim_out completion OR manual toggle)"
-- This matches how the UI displays installed status
-- =====================================================
...
-- ===========================================================================
-- TRIM STAGES: NOW BASED ON EQUIPMENT INSTALLED STATUS
-- ===========================================================================
-- Equipment is considered "installed" if:
--   1. pe.installed = true (manual toggle for items without wire drops), OR
--   2. Equipment is linked to a wire drop that has trim_out stage completed
-- ===========================================================================
trim_stages_calc AS (
  SELECT
    pe.project_id,
    ...
    COUNT(*) FILTER (
      ...
      OR EXISTS (
        -- Equipment linked to a wire drop with completed trim_out
        SELECT 1
...
SELECT 'Trim stages now based on equipment installed status!' as status;
