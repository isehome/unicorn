-- =====================================================
-- FIX TRIM CALCULATION: ONLY ROOM_END AUTO-INSTALLS
-- =====================================================
-- Updates the equipment_trim_calc to only auto-mark equipment
-- as "installed" when linked via room_end to a completed wire drop.
--
-- Head-end equipment (many:1 relationship, like switches) must
-- be manually toggled to installed - they don't auto-complete
-- when their wire drops are trimmed out.
-- =====================================================
...
--   - pe.installed = true (manual toggle for wireless/head-end items), OR
--   - Equipment linked via ROOM_END to wire drop with completed trim_out
--     (Head-end equipment does NOT auto-install - requires manual toggle)
-- ===========================================================================
...
-- FIXED: Only room_end links auto-install equipment
  ...
  SELECT
    ...
    COUNT(*) FILTER (
      ...
      OR EXISTS (
        ...
        AND wdel.link_side = 'room_end'  -- ONLY room_end auto-installs
        AND wds.stage_type = 'trim_out'
        AND wds.completed = true
      )
...
SELECT 'Trim stages now only auto-installs room_end equipment!' as status;
