...
function generateReportHtml({ project, milestones, milestoneDates, externalIssues, internalIssues, blockedIssues, issuePortalLinks, todos, includeTodos }) {
  // Build milestone date lookup - capture raw dates for debugging
  const dateMap = {};
  const debugRawDates = [];
  (milestoneDates || []).forEach(m => {
    debugRawDates.push(`${m.milestone_type}: target="${m.target_date}" actual="${m.actual_date}"`);
...
<body>
  <div class="container">
    ...
    <div class="footer">
      ...
      <!-- DEBUG: Show raw dates from database to diagnose timezone issue -->
      <div style="margin-top: 20px; padding: 10px; background: #1a1a1a; border: 2px solid #ffcc00; font-family: monospace; font-size: 11px; color: #ffcc00; text-align: left; white-space: pre-wrap;">
DEBUG RAW DATES FROM SUPABASE:
${debugRawDates.join('\n')}
      </div>
    </div>
  </div>
