...
const { getModelForService, getServiceConfig, logModelUsage } = require('../_aiConfig');
const fs = require('fs');
const path = require('path');

// Severity/label thresholds that trigger deep analysis with reasoning model
const DEEP_ANALYSIS_SEVERITIES = ['critical', 'high'];
const DEEP_ANALYSIS_LABELS = ['auth', 'api', 'data', 'security', 'state', 'oauth', 'database'];
...
Return ONLY valid JSON, no markdown formatting.`;
  ...
  // Track which model did the triage
  const triageConfig = getServiceConfig('BUG_ANALYZER');

  // Determine if this bug needs deep analysis from the reasoning model
  const needsDeepAnalysis = shouldEscalateToDeepAnalysis(analysis);

  let deepAnalysis = null;
  let deepAnalysisModel = null;
  let deepTokenUsage = null;

  if (needsDeepAnalysis) {
    console.log(`[BugAnalyze] Escalating to deep analysis (severity: ${analysis.severity}, labels: ${(analysis.labels || []).join(', ')})`);
    try {
      const deepResult = await performDeepAnalysis(bugReport, analysis, codeContext);
      deepAnalysis = deepResult.deepAnalysis;
      deepAnalysisModel = deepResult.modelId;
      deepTokenUsage = deepResult.tokenUsage;

      // Merge the deeper fix_prompt back into the main analysis if it's better
      if (deepAnalysis && deepAnalysis.fix_prompt) {
        analysis.fix_prompt = deepAnalysis.fix_prompt;
        analysis.root_cause = deepAnalysis.root_cause || analysis.root_cause;
        analysis.suggested_files = deepAnalysis.suggested_files || analysis.suggested_files;
        analysis.testing_steps = deepAnalysis.testing_steps || analysis.testing_steps;
        // Boost confidence if deep analysis agrees
        if (deepAnalysis.confidence) {
          analysis.confidence = Math.max(analysis.confidence || 0, deepAnalysis.confidence);
        }
      }
    } catch (deepErr) {
      console.error('[BugAnalyze] Deep analysis failed, using triage results:', deepErr.message);
    }
  }

  // Add metadata including token usage and model info
  return {
    ...analysis,
    ...
    analysis_model: triageConfig.modelId,
    deep_analysis: deepAnalysis ? JSON.stringify(deepAnalysis) : null,
    deep_analysis_model: deepAnalysisModel,
    deep_token_usage: deepTokenUsage,
    escalated_to_deep: needsDeepAnalysis,
  };
}

/**
 * Determine if a bug needs deep analysis from the reasoning model
 */
function shouldEscalateToDeepAnalysis(triageResult) {
  // Escalate based on severity
  if (DEEP_ANALYSIS_SEVERITIES.includes(triageResult.severity)) {
    return true;
  }

  // Escalate based on labels that indicate complex issues
  const labels = (triageResult.labels || []).map(l => l.toLowerCase());
  if (labels.some(l => DEEP_ANALYSIS_LABELS.includes(l))) {
    return true;
  }

  // Escalate if triage confidence is low (model is unsure)
  if (triageResult.confidence && triageResult.confidence < 0.5) {
    return true;
  }
...
/**
 * Perform deep analysis with the reasoning model (Gemini 3.1 Pro)
 * Only called for complex bugs that need deeper code understanding
 */
async function performDeepAnalysis(bugReport, triageResult, codeContext) {
  const deepModel = getModelForService('BUG_ANALYZER_DEEP');
  const deepConfig = getServiceConfig('BUG_ANALYZER_DEEP');

  const deepPrompt = `You are a senior software architect performing deep code analysis on a bug report that was flagged as complex by our triage system.

## Triage Summary (from initial analysis)
- **Summary:** ${triageResult.summary}
- **Severity:** ${triageResult.severity}
- **Root Cause (initial):** ${triageResult.root_cause}
- **Confidence:** ${triageResult.confidence}
- **Labels:** ${(triageResult.labels || []).join(', ')}
...
## Console Errors
\`\`\`
${bugReport.console_errors?.length > 0 ? bugReport.console_errors.join('\n\n') : 'No console errors captured.'}
\`\`\`
...
The triage system flagged this as needing deeper analysis. Think step by step:

1. **Verify the triage**: Is the initial root cause assessment correct, or is there a deeper issue?
2. **Trace the full call chain**: Follow the code path from user action → frontend → API → database
3. **Identify ALL affected files**: Not just the obvious ones, but related services, middleware, shared utilities
4. **Write production-ready fix instructions**: Specific enough that another developer (or AI agent) can implement the fix without guessing

Return a JSON object:
{
  "root_cause": "Detailed technical root cause after deeper analysis",
  "fix_prompt": "Step-by-step fix instructions with exact file paths, line numbers, and code changes needed",
  "suggested_files": [
    {"file": "src/path/to/file.js", "line": 123, "description": "What to change and why"}
  ],
  "testing_steps": ["Step 1", "Step 2"],
  "confidence": 0.85,
  "complexity": "simple|moderate|complex",
  "triage_accuracy": "correct|partially_correct|incorrect",
  "additional_context": "Any important context the triage missed"
}

Return ONLY valid JSON, no markdown formatting.`;

  const parts = [{ text: deepPrompt }];
  ...
  if (bugReport.screenshot_base64) {
    try {
      ...
      console.error('[BugAnalyze] Failed to process screenshot for deep analysis:', err.message);
    }
  }

  const result = await deepModel.generateContent(parts);
  const response = await result.response;
  const text = response.text();
  ...
  logModelUsage('BUG_ANALYZER_DEEP', usageMetadata);
  ...
  try {
    ...
    console.error('[BugAnalyze] Failed to parse deep analysis response:', text.slice(0, 500));
    deepAnalysis = {
      root_cause: 'Deep analysis response could not be parsed',
      fix_prompt: text,
      confidence: 0.3,
      complexity: 'complex',
      triage_accuracy: 'unknown'
    };
  }

  return {
    deepAnalysis,
    modelId: deepConfig.modelId,
    tokenUsage,
  };
}
